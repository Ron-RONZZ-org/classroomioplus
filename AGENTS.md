...
- **Pre-existing lint/test issues (not environment problems):** `pnpm --filter @cio/api lint` fails (missing ESLint v9 `eslint.config.*`); `pnpm --filter @cio/dashboard lint` runs but reports pre-existing errors; api `vitest run` passes the full unit suite (~138 tests) and integration tests pass when a test DB is reachable (run `vitest run` explicitly — the `pnpm test` script is watch mode and never exits); dashboard tests migrated from jest to vitest and pass. The pre-commit gate `pnpm format:check` passes.
- The optional `@cio/storybook` build fails on an unresolved `@lucide/svelte/icons/bot` import; it does not affect api/dashboard.
- **Worktree `node_modules` symlink + Vite `fs.allow`:** Git worktrees symlink `node_modules` to the parent checkout (see [Worktree Node Modules](#worktree-node-modules)). Vite follows symlinks to their real paths, and its `server.fs.allow` security check blocks serving files outside the project directory. If the dashboard SPA loads HTML but fails to hydrate (browser console shows `Failed to fetch dynamically imported module` pointing to a path outside the worktree), the `vite.config.ts` `server.fs.allow` list needs the parent checkout root added. The config detects symlinked `node_modules` and resolves the real path automatically — but if you move the parent checkout or use a different directory structure, verify `getFsAllow()` in `vite.config.ts` resolves correctly. Debug by checking the real path: `readlink -f apps/dashboard/node_modules`.
- **`@cio/db` build failure due to partial `dist/`:** The `@cio/db` package has a pre-existing TypeScript error in `packages/db/src/queries/course-import/course-import.ts`. Building `@cio/db` after a `pnpm install` in a fresh worktree may fail partway through, leaving `dist/` incomplete. Symptoms: the API server crashes at startup with `ERR_MODULE_NOT_FOUND: Cannot find module '.../packages/db/dist/auth/index.json'`. Fix: build `@cio/db`'s dependencies first (`@cio/utils`, `@cio/email`) if they aren't already built, then retry `pnpm --filter @cio/db build`. The TS error does not affect runtime because the failing file is only used by the course-import feature (which may itself be broken). Alternatively, copy `dist/` from the parent checkout: `cp -r /path/to/parent/packages/db/dist packages/db/dist`.
- **Stale dev-server PID files:** After restarting the API or dashboard dev server, old PID files in `/tmp/cio-*.pid` may reference a dead process. The `scripts/dev.sh stop` command kills by PID from these files. If the server won't start because `dev.sh` says "process still running", check `ps aux | grep tsx` and `ps aux | grep vite` for zombie processes. Fix: `rm -f /tmp/cio-*.pid` and kill any leftover `tsx`/`vite` processes manually.
- **SvelteKit `mode: 'auto'` CSP blocks inline event handlers in dev:** The app's CSP (`svelte.config.js`) uses `script-src` with `'unsafe-hashes'` but without `sha256-...` entries. SvelteKit's `mode: 'auto'` should generate these hashes but does not under Vite dev. The symptom is that inline event handlers (from Vite HMR or third-party widgets like the Google login button) are silently blocked, causing form submission and other interactions to fail. There is a report-only policy that logs violations without blocking, alongside an enforcement policy that does block. If you encounter mysterious login failures during dev testing, check the browser console for `Content Security Policy` violations. The workaround is to add `'unsafe-inline'` to `script-src` in both `directives` and `reportOnly` in `svelte.config.js`.
