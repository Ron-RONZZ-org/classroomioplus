...
import { seedOrganizationPlan } from '@db/utils/seed/organizationPlan';
...
// Self-hosted single-org: one organization (udemy-test) with admin + student.
...
Flags:
  ...
  --organization-plan         Seed the self-hosted ENTERPRISE plan
  --groups                    Seed groups
...
const seedFunctions = {
  ...
  'organization-plan': async () => {
    console.log('📝 Seeding organization plan (ENTERPRISE, self-hosted)...');
    await seedOrganizationPlan({ orgId: TEST_ORG_ID });
  },
  groups: async () => {
