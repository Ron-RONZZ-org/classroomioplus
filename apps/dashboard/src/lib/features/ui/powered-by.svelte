<script lang="ts">
  ...
  import { APP_NAME, BRAND_ROOT_DOMAIN, TENANT_ROOT_DOMAIN } from '$lib/utils/constants/branding';
...
{:else if $isFreePlan}
  <a
    href={`https://${BRAND_ROOT_DOMAIN}?utm_source=${$currentOrg.siteName}.${TENANT_ROOT_DOMAIN}`}
    target="_blank"
  ...
  >
    ...
    >
      ...
      {#if !showOnlyLogo}
        {$t('course.navItem.landing_page.powered_by')} {APP_NAME}
      {/if}
    </span>
  </a>
{/if}
